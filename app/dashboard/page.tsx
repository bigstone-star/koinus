...
export default function Dashboard() {
  ...
  if(loading)return <div className="min-h-screen flex items-center justify-center bg-slate-100"><div className="w-8 h-8 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"/></div>
  return (
    <div className="min-h-screen bg-slate-100 max-w-lg mx-auto pb-10">
      <div className="bg-[#1a1a2e] px-5 pt-12 pb-6 flex items-center justify-between">
        ...
        <button onClick={signOut} className="flex items-center gap-1.5 text-white/40 text-[12px] border border-white/20 px-3 py-1.5 rounded-lg"><LogOut size={13}/>로그아웃</button>
      </div>
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100">
          <a href="/dashboard/profile" className="flex items-center gap-3 px-4 py-3.5 hover:bg-slate-50"><span className="text-xl">✏️</span><span className="text-[14px] font-bold text-slate-700 flex-1">업소 정보 수정</span><ChevronRight size={16} className="text-slate-300"/></a>
          <a href="/pricing" className="flex items-center gap-3 px-4 py-3.5 hover:bg-slate-50"><span className="text-xl">⭐</span><span className="text-[14px] font-bold text-slate-700 flex-1">VIP 요금제</span><ChevronRight size={16} className="text-slate-300"/></a>
        </div>
      </div>
    </div>
  )
