...
import { createBrowser, CATEGORIES, CAT_ICONS, CAT_BG, type Business } from '@/lib/supabase'
import { Heart, Phone, Globe, Star, Search, User, X } from 'lucide-react'

const supabase = createBrowser()

function CategoryGrid({ selected, onSelect, counts }: { selected:string; onSelect:(c:string)=>void; counts:Record<string,number> }) {
  return (
      ...
      <div className="grid grid-cols-3 gap-2">
        {CATEGORIES.map(cat => (
          <button key={cat.name} onClick={() => onSelect(cat.name)}
            className={`flex items-center gap-2 px-3 py-2.5 rounded-lg border-[1.5px] transition-all active:scale-[.97] text-left ${selected===cat.name?'border-indigo-500 bg-indigo-50':'border-slate-200 bg-white'}`}>
...
function BusinessCard({ b, fav, onFav, onClick }: { b:Business; fav:boolean; onFav:()=>void; onClick:()=>void }) {
  const addrShort = b.address?.split(',').slice(0,-2).join(',').trim()??b.address
  const tierColor = b.vip_tier==='premium'?'bg-amber-400':b.vip_tier==='pro'?'bg-amber-300':'bg-amber-200'
  return (
    <div onClick={onClick} className={`bg-white rounded-xl border px-4 py-3.5 flex gap-3 cursor-pointer active:scale-[.99] ${b.is_vip?'border-amber-300 bg-amber-50/30':'border-slate-200'}`}>
      ...
      <div className="flex-1 min-w-0">
        {(b.is_vip||b.category_sub)&&(
          <div className="flex gap-1 mb-1 flex-wrap">
            {b.is_vip&&<span className={`text-[9px] font-black px-1.5 py-0.5 rounded text-amber-900 ${tierColor}`}>⭐{(b.vip_tier??'').toUpperCase()}</span>}
        ...
        <div className="flex items-center gap-2.5 mt-1 flex-wrap">
          {b.rating&&b.rating>0?(<span className="flex items-center gap-0.5 text-[12px] font-bold text-slate-800"><Star size={11} className="fill-amber-400 stroke-amber-400"/>{b.rating.toFixed(1)}<span className="text-[11px] font-normal text-slate-400 ml-0.5">({b.review_count?.toLocaleString()})</span></span>):null}
...
function BusinessDrawer({ b, fav, onFav, onClose }: { b:Business; fav:boolean; onFav:()=>void; onClose:()=>void }) {
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-end" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="bg-white rounded-t-2xl w-full max-h-[90vh] overflow-y-auto pb-10">
        <div className="flex items-center justify-between px-5 pt-4 pb-0">
          ...
          <div/><button onClick={onClose} className="ml-auto p-1 text-slate-400"><X size={20}/></button>
        </div>
        <div className="px-5 pt-2 pb-4 border-b border-slate-100">
          ...
          <div className="text-[22px] font-extrabold text-slate-900 mt-2">{b.name_kr||b.name_en}</div>
          {b.rating&&b.rating>0?(<div className="flex items-center gap-2 mt-2"><span className="text-amber-400">{'★'.repeat(Math.round(b.rating))}</span><span className="font-bold">{b.rating.toFixed(1)}</span><span className="text-[13px] text-slate-400">리뷰 {b.review_count?.toLocaleString()}개</span></div>):null}
          {b.description_kr&&<p className="text-[13px] text-slate-600 mt-3 bg-slate-50 rounded-lg px-3 py-2.5">{b.description_kr}</p>}
        </div>
        <div className="px-5 py-2 divide-y divide-slate-50">
          {b.address&&<div className="flex gap-3 py-3"><div className="w-9 h-9 rounded-lg bg-indigo-50 flex items-center justify-center flex-shrink-0">📍</div><div><div className="text-[10px] font-bold text-slate-400 uppercase mb-1">주소</div><div className="text-[14px]">{b.address}</div></div></div>}
          {b.phone&&<div className="flex gap-3 py-3"><div className="w-9 h-9 rounded-lg bg-indigo-50 flex items-center justify-center flex-shrink-0">📞</div><div><div className="text-[10px] font-bold text-slate-400 uppercase mb-1">전화</div><a href={`tel:${b.phone}`} className="text-[14px] font-semibold text-indigo-600">{b.phone}</a></div></div>}
  ...
  return (
    <div className="min-h-screen bg-slate-100 max-w-lg mx-auto">
      <header className="bg-[#1a1a2e] sticky top-0 z-40 shadow-lg">
        <div className="flex items-center justify-between px-4 pt-4 pb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {user?(<a href="/dashboard" className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center"><User size={16} className="text-white"/></a>):(<a href="/auth/login" className="text-[12px] font-bold text-white/70 bg-white/10 px-3 py-1.5 rounded-lg border border-white/20">로그인</a>)}
          </div>
        </div>
      ...
      <main className="px-3 py-2.5 pb-24 space-y-2">
        {!user&&(<a href="/auth/login" className="block bg-gradient-to-r from-indigo-600 to-violet-600 rounded-xl px-4 py-3.5 mb-2"><div className="text-white font-bold text-[14px]">🏢 내 업소를 무료로 등록하세요!</div><div className="text-white/70 text-[12px] mt-0.5">구글 로그인 → 업소 등록 → VIP 업그레이드로 상단 노출</div></a>)}
        {loading?(<div className="flex items-center justify-center py-20"><div className="w-8 h-8 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"/></div>):businesses.length===0?(<div className="text-center py-20 text-slate-400 text-[15px]">검색 결과가 없습니다</div>):(businesses.map(b=>(<BusinessCard key={b.id} b={b} fav={favs.includes(b.id)} onFav={()=>toggleFav(b.id)} onClick={()=>setSelected(b)}/>)))}
      </main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-lg mx-auto bg-white border-t border-slate-200 flex z-30">
        <a href="/" className="flex-1 flex flex-col items-center gap-1 py-2.5 text-indigo-600"><span className="text-xl">🏠</span><span className="text-[10px] font-bold">홈</span></a>
        <a href="/register" className="flex-1 flex flex-col items-center gap-1 py-2.5 text-slate-400"><span className="text-xl">➕</span><span className="text-[10px] font-bold">업소등록</span></a>
        <a href="/pricing" className="flex-1 flex flex-col items-center gap-1 py-2.5 text-slate-400"><span className="text-xl">⭐</span><span className="text-[10px] font-bold">VIP</span></a>
        <a href={user?'/dashboard':'/auth/login'} className="flex-1 flex flex-col items-center gap-1 py-2.5 text-slate-400"><span className="text-xl">👤</span><span className="text-[10px] font-bold">{user?'내 정보':'로그인'}</span></a>
      </nav>
    </div>
  )
}