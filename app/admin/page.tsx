...
export default function AdminPage() {
  ...
  const loadList = useCallback(async (searchTerm?: string) => {
    ...
    if (term.trim()) {
      q = q.or(`name_kr.ilike.%${term}%,name_en.ilike.%${term}%,category_main.ilike.%${term}%,address.ilike.%${term}%`)
    }
  ...
  const addCat = async () => {
    ...
    const { error } = await sb.from('categories').insert({
      name: newCatName.trim(), icon: newCatIcon, sort_order: maxOrder + 1, is_active: true
    })
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            ...
            <div className="flex gap-2">
              <input value={newCatIcon} onChange={e => setNewCatIcon(e.target.value)}
                placeholder="아이콘" className="w-14 border border-slate-200 rounded-lg px-2 py-2 text-center text-[18px]" />
              ...
              <button onClick={addCat} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-[13px] font-bold">추가</button>
            </div>
          </div>
          {catLoading
              ...
              <div key={c.id} className={`bg-white rounded-xl border p-4 flex items-center gap-3 ${!c.is_active ? 'opacity-50 border-slate-100' : 'border-slate-200'}`}>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => deleteCat(c.id, c.name)}
                    className="text-[11px] font-bold px-3 py-1.5 rounded-lg bg-red-50 text-red-500">삭제</button>
                </div>
              </div>
            ))
          }
        ...
        <div className="px-4 space-y-2">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-3">
            <div className="flex gap-2">
              ...
              {search && (
                <button onClick={() => { setSearch(''); loadList('') }}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {!b.approved && <button onClick={() => approve(b.id)} className="bg-green-500 text-white text-[11px] font-bold py-1.5 px-3 rounded-lg">✅ 승인</button>}
                        <button onClick={() => toggleVip(b)} className={`text-[11px] font-bold py-1.5 px-3 rounded-lg ${b.is_vip ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-700'}`}>
                          {b.is_vip ? 'VIP 해제' : '⭐ VIP'}
                        </button>
                        <button onClick={() => deactivate(b.id)} className="text-[11px] font-bold py-1.5 px-3 rounded-lg bg-slate-100 text-slate-500">🗑</button>
                      </div>
                    </div>
                  </div>
                </div>
