...
export default function PricingPage() {
  ...
  const go = async (tier: string) => {
    ...
    if (biz.is_vip && biz.vip_tier === tier) { setMsg('이미 ' + tier.toUpperCase() + ' 플랜을 사용 중입니다'); setTimeout(() => setMsg(''), 3000); return }
    ...
    try {
      const res = await fetch('/api/subscribe', {
        ...
        body: JSON.stringify({ tier, billing, businessId: biz.id, userId: user.id }),
      })
      ...
      <div className="px-4 space-y-4">
        {PLANS.map(p => (
          <div key={p.tier} className={`bg-white rounded-2xl border-2 p-5 relative ${p.color}`}>
            ...
            <button onClick={()=>go(p.tier)} disabled={loading===p.tier} className={`w-full py-3.5 rounded-xl text-[14px] font-bold transition-all disabled:opacity-60 ${p.tier==='pro'?'bg-indigo-600 text-white':p.tier==='premium'?'bg-amber-500 text-white':'bg-slate-100 text-slate-700'}`}>
