...
const PLANS = [
  ...
  { tier:'pro', name:'Pro', price:49, yp:41, color:'border-indigo-500', badge:'인기', features:['카테고리 최상단 노출','PRO 배지 + 하이라이트','한국어 소개글','사진 1장','주간 통계','SNS 연동'] },
  { tier:'premium', name:'Premium', price:79, yp:66, color:'border-amber-400', badge:'최고', features:['전체 최상단 노출','PREMIUM 골드 배지','소개글 무제한','사진 5장','실시간 통계','홈 배너','전용 담당자'] },
]
export default function PricingPage() {
  ...
  const go = async (tier: string) => {
    ...
    alert(tier.toUpperCase() + ' 플랜 신청 접수!\n담당자가 연락드립니다.\nadmin@kyocharo.com')
  }
  return (
        ...
        {PLANS.map(p => (
          <div key={p.tier} className={`bg-white rounded-2xl border-2 p-5 relative ${p.color}`}>
            ...
            <ul className="space-y-2 mb-5">
              {p.features.map(f=><li key={f} className="flex items-start gap-2 text-[13px] text-slate-600"><span className="text-indigo-500 flex-shrink-0">✓</span>{f}</li>)}
            </ul>
