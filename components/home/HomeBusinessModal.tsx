...
export default function HomeBusinessModal({
  ...
  const safeRelatedPosts = Array.isArray(relatedCommunityPosts)
    ? relatedCommunityPosts
    : []
  const safeTags = Array.isArray(reviewForm?.tags) ? reviewForm.tags : []
      ...
      <div className="bg-white rounded-t-2xl w-full max-h-[90vh] overflow-y-auto pb-10">
          ...
          {Number(sel.rating || 0) > 0 && (
            <div className="mt-2">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold">{Number(sel.rating || 0).toFixed(1)}</span>
        ...
        <div className="px-5 pt-4 border-t border-slate-100 mt-4">
          ...
          {relatedPostsLoading ? (
            ...
            <div className="space-y-2 mb-4">
              {safeRelatedPosts.map((p) => (
                <Link key={p.id} href={`/community/${p.region}/${p.id}`} className="block px-3 py-2 rounded-lg border border-slate-100 hover:bg-slate-50">
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <div className="text-[12px] text-slate-400 mt-0.5">
                {safeReviews.length > 0 ? `평균 ★${Number(avgRating || 0).toFixed(1)} · ${safeReviews.length}개` : '아직 리뷰가 없습니다'}
              </div>
            </div>
          </div>
          ...
          {user ? (
                  ...
                  {REVIEW_TAGS.map((tag) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-[12px] font-bold border ${active ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200'}`}
                      >
                        {tag}
                      </button>
                    )
            ...
            <Link href="/auth/login" className="block bg-slate-50 rounded-xl border border-slate-200 p-4 mb-4 text-[13px] text-slate-600">
              리뷰 작성은 로그인 후 가능합니다.
            </Link>
          )}
        ...
        <div className="flex gap-2 px-5 pt-2">
          ...
          {sel.website && (
            <a href={sel.website} target="_blank" rel="noreferrer" className="flex-1 bg-indigo-50 text-indigo-600 py-3.5 rounded-xl text-[14px] font-bold text-center">
              🌐 홈페이지
            </a>
          )}
        </div>
